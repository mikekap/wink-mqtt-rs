...
use hyper::{Server, Request, Body, Response, Method};
...
impl HttpServer {
    pub fn new(config: &Config, controller: Arc<dyn DeviceController>) -> Arc<HttpServer> {
        ...
        let server = Server::bind(&SocketAddr::from(([127, 0, 0, 1], config.http_port.unwrap())))
            .tcp_nodelay(true)
            .http1_only(true)
            .serve(handler)
    ...
    async fn handler(self: Arc<Self>, request: Request<Body>) -> Result<Response<Body>, hyper::Error> {
